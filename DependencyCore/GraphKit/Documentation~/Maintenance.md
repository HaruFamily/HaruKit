...
| `Runtime/Contracts/` | `GraphContracts`（`IGraphHead`、`IOrphanPool`、`ITokenOwner`、`IPropertyOwner`、`IGraphDocument`、`HGCapabilities`、`ISequentialActionContainer`、`IGraphNodeOwner`、`ITypedFormulaNode`、`IGraphSink`、`IGraphExecutionDocument`）、`GraphDiagnostics`、`AssetGraphSchema`、`ActionNodeAttribute`（`[HG*]` 屬性）、`IGraphOwner`、`IExternalTokenKeys` |
...
| `Editor/VisualEditor/Project/` | `HGTypeCatalog`（可建立型別與選單）、資產索引與落點、Owner 索引、引用索引 |
...
- 可編輯的圖實作 `IGraphDocument`：roots、驗證旗標、`MarkDirty`／`Verify`／`DeepCopy`、`PackType`、`ItemSlotType`、root 識別值與用詞（`RootChip`、`RootNoun`、`WindowTitle`），以及 `HGCapabilities`。
...
5. **存檔必須通過驗證。** 視窗存檔、`HGModel.Save`、非視窗 `Commit` 都要求診斷與文件 `Verify()` 通過。沒有草稿存檔，也不可為了讓存檔通過而放寬驗證。
...
13. **純版面操作不進文件。** 庫的停駐、排序、顯示、欄寬等偏好存在 EditorPrefs（`HaruGraph.*` 鍵），不標 Dirty、不進 Undo。
...
- 快照式，掛在 `HGModel.MarkDirty()`：0.4 秒內連續修改合併成一步，上限 40 步。Undo／Redo 會整份換掉 `Data`，之後要依穩定識別重新解析焦點。
...
- 提交順序：遺失型別清理 → DeepCopy → 副本 `MarkDirty()`／`Verify()` → 通過 → 版本檢查 → 寫回。驗證、衝突或寫入失敗都不清除 Dirty。
...
- Slot 族：`[HGKind(name, group, priority)]`。建立公式、新增 ProtoProperty、新增 Token 三個選單共用 `HGTypeCatalog.FormulaKindOptions`；group 用 `/` 分多層，priority 越小越前面。同路徑同名的族附完整型別與組件名稱消歧義。Group／Priority 不影響族身分或相容性。
...
- [ ] 資料修改都經過 `Invalidate()`；純視覺狀態（折疊、顯示開關、版面）只設 graphDirty＋Repaint，不標未存檔

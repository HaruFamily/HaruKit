...
/// </summary>
// 三種欄位（Formula／Action／Property）真正共通的只有「指著一顆節點」與「收不收得下某個來源」。
// 其餘（結果型別、座標、候選池、包的方向）各自宣告，不往上堆——
// 上移一個只有一種欄位用得到的成員，等於逼其餘幾種長出假的實作。
//
...
public abstract class GraphSlotBase
{
    ...
    // 四個 Accepts 在這一層是 virtual 而不是 abstract：目錄欄位只收包，寫入目標欄位只收 Property，
    // 逼它們實作三個永遠回 false 的成員只會讓「它其實不收這些」變得不明顯。
    ...
    /// <summary>能不能接這個 Property。求值欄位與寫入目標欄位都用它，方向由欄位自己的型別區分。</summary>
    public virtual bool AcceptsProperty(GraphProperty property) => false;
}

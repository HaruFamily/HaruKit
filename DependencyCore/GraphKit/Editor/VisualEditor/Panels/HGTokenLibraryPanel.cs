...
/// <summary>畫一次變數庫需要的全部資料。面板不認識 model 或 focus，只認這份快照。</summary>
public struct HGTokenLibraryView
{
    /// <summary>目前這張畫布的對外端點。資產焦點下是那個資產的參數介面。</summary>
    public List<HGToken> Tokens;

    /// <summary>正在編輯哪一個變數；null＝不在變數焦點。</summary>
    public GraphEndpoint FocusedEndpoint;
}

/// <summary>
/// 變數庫對外的命令。面板只回報使用者做了什麼，怎麼改圖由視窗決定。
/// </summary>
public struct HGTokenLibraryCommands
{
    /// <summary>改名。回傳 false＝名稱不合法，就地改名會維持編輯狀態。</summary>
    public Func<GraphEndpoint, string, bool> Rename;

    /// <summary>點一筆：進去編它，或再點目前這筆＝退出。進出判斷在視窗。</summary>
    public Action<GraphEndpoint> Activate;

    /// <summary>拖到「＋」上放開：複製這一個（內容一起複製）。</summary>
    public Action<GraphEndpoint> Duplicate;

    /// <summary>拖到「－」上放開，或直接按「－」刪掉目前編輯中的那一個。</summary>
    public Action<GraphEndpoint> Remove;

    /// <summary>按「＋」：開型別選單。</summary>
    public Action Create;

    /// <summary>這個變數有沒有驗證問題；reason 為 null＝沒有。</summary>
    public Func<HGToken, (string reason, bool isError)> IssueOf;
}

/// <summary>
/// 左欄變數庫：這張圖有哪些對外端點。新增、改名、刪除都在這裡，點一筆進入它自己的畫布。
/// </summary>
// 拖放區面板。搜尋字與捲動自己持有；「正在拖誰」與「誰正在被改名」向框架借。
// 兩顆按鈕同時是拖曳落點（拖變數上去＝複製／刪除），所以它們也得走框架的拖曳狀態。
public sealed class HGTokenLibraryPanel
{
        ...
        HGInlineRename inlineName, HGLibraryDrag drag)
    {
        ...
        for (int i = 0; i < shown.Count; i++)
        {
            ...
            bool isFocus = ReferenceEquals(view.FocusedEndpoint, endpoint);
    ...
    {
        var e = Event.current;
        bool dropping = drag.DroppingEndpoint;
        ...
        HGTokenLibraryCommands cmd, HGLibraryDrag drag)
    {
        ...
        bool hasFocusEndpoint = view.FocusedEndpoint != null;
        ...
        if (dropping && hover && e.rawType == EventType.MouseUp)
        {
            cmd.Remove(drag.Endpoint);
            drag.Clear();
            e.Use();
            return;
        }
        if (clicked && hasFocusEndpoint) cmd.Remove(view.FocusedEndpoint);
    }
}

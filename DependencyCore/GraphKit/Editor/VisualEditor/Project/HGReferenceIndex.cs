...
/// <summary>
/// 「誰引用了這個共用資產」的反向索引，從 <see cref="HGOwnerIndex"/> 現算。
///
...
/// </summary>
[InitializeOnLoad]
public static class HGReferenceIndex
{
    ...
    static HGReferenceIndex() => EditorApplication.projectChanged += Invalidate;
    ...
    {
        cache = null;
        HGOwnerIndex.Invalidate();
    }

    /// <summary>立刻重掃（使用者手動按「重新掃描」）。</summary>
    public static void Refresh()
    {
        HGOwnerIndex.Refresh();
        cache = Build();
    }

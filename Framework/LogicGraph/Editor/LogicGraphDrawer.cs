...
/// </summary>
// 註冊在開放泛型上，任何含 LogicGraph<,> 欄位的 Owner 都吃得到，不必逐個 Owner 寫 CustomEditor。
// 節點圖是唯一編輯點：Inspector 展開巢狀清單只會提供第二條會打架的編輯路徑。
// 畫成卡片而不是一顆裸按鈕，是因為這一格背後是整套編輯系統，外觀要對得起它的份量。
[CustomPropertyDrawer(typeof(LogicGraph<,>), true)]
public class LogicGraphDrawer : PropertyDrawer
{

...
/// <summary>
/// 資產焦點的復原歷程。資產是獨立存檔交易、不進 Owner 的工作副本，所以 <see cref="LGModel"/> 的
/// Undo 堆疊蓋不到它，得自己記一份。行為刻意與那邊一致：快照式、0.4 秒內連續修改合併成一步、上限 40 步。
/// </summary>
// 快照式而不是逐項記錄：圖是 SerializeReference 多型樹，節點數是幾十個等級，整份抄最直接。
public class LGAssetHistory
{
    private const int UndoLimit = 40;
    private const double MergeWindow = 0.4;

    private readonly List<LGAssetSnapshot> undoStack = new();
    private readonly List<LGAssetSnapshot> redoStack = new();
    private LGAssetSnapshot baseline;                 // 上一個記錄點的狀態（＝本次修改前的狀態）
    ...
    public void Reset(LGAssetSnapshot current)
    {
        undoStack.Clear();
        redoStack.Clear();
        baseline = current;
    ...
    public void Record(LGAssetSnapshot current)
    {
    ...
    public LGAssetSnapshot Undo(LGAssetSnapshot current)
    {
    ...
    public LGAssetSnapshot Redo(LGAssetSnapshot current)
    {
    ...
    public void Rebase(LGAssetSnapshot current) => baseline = current;
}

...
/// <summary>
/// 「誰引用了這個共用資產」的反向索引，從 <see cref="LGOwnerIndex"/> 現算。
///
...
/// </summary>
[InitializeOnLoad]
public static class LGReferenceIndex
{
    ...
    static LGReferenceIndex() => EditorApplication.projectChanged += Invalidate;
    ...
    {
        cache = null;
        LGOwnerIndex.Invalidate();
    }

    /// <summary>立刻重掃（使用者手動按「重新掃描」）。</summary>
    public static void Refresh()
    {
        LGOwnerIndex.Refresh();
        cache = Build();
    }
    ...
    private static Dictionary<ScriptableObject, List<ScriptableObject>> Build()
    {
        ...
        foreach (var entry in LGOwnerIndex.Entries)
        {
            var owner = entry.Owner;
            if (owner == null) continue;

            var system = LGModel.FindSystemField(owner)?.GetValue(owner);
            if (system == null) continue;

            foreach (var asset in LGModel.ReferencedAssetsOfSystem(system))
            {

    ...
    public partial class AssetPipeline
    {
        ...
        internal void RunPipelineAssets()
        {
            ...
            bool confirmed = EditorUtility.DisplayDialog(
                "執行管線",
                $"確定執行 {pipelineAssets.Count} 個管線資產？\n此操作會實際修改資產，無法自動復原（請確認已 commit）。",
                "執行",
                "取消");
        ...
        internal string ExecutePipelineAssets(string actionName)
        {
            ...
            try
            {
                foreach (IPipelineAsset pipelineAsset in pipelineAssets)
                {
                    if (pipelineAsset == null)
                    {
                        errorCount++;
                        string message = $"[AssetPipeline] {actionName} 管線資產失敗：元素為 null。";
                    ...
                    try
                    {
                        pipelineAsset.Execute();
                        successCount++;
                    }
                    catch (Exception ex)
                    {
                        errorCount++;
                        string message = $"[AssetPipeline] {actionName} 管線資產失敗：{pipelineAsset.GetType().Name}\n{ex}";
